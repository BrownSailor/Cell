...
std::string assemble_post(Node *root, const std::unordered_map<std::string, Node *> &scope)
{
    std::string expr = "";
...
std::string assemble_expr(Node *root, const std::unordered_map<std::string, Node *> &scope)
{
    ...
    else if (root->token.type == Token::TOK_PRINT)
    {
        ...
        else if (root->children.front()->children.size() &&
                 root->children.front()->children.front()->is_idx)
        {
    ...
    else if (root->token.type == Token::TOK_POST_INC ||
             root->token.type == Token::TOK_POST_DEC)
    {
        expr += assemble_post(root, scope) + ";\n";
    }
